...
func CheckIfHTTPSOnly(checkConfig commons.CheckConfig, d []types.DistributionSummary, testName string) {
	...
	check.InitCheck("Cloudfronts only allow HTTPS or redirect to HTTPS", "Check if all cloudfront distributions are HTTPS only", testName, []string{"Security", "Good Practice", "HDS"})
