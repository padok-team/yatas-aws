...
func CheckPasswordPolicy(checkConfig commons.CheckConfig, passwordPolicy *types.PasswordPolicy, testName string) {
	...
	check.InitCheck("IAM password policy must be defined and enforced", "Check if the password policy is strong", testName, []string{"Security", "Good Practice", "HDS"})
	var errors []string

	if passwordPolicy == nil {
		errors = append(errors, "No password policy found")
	} else {
